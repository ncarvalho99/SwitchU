...
The pass also exposed one smaller CPU/allocation inefficiency rather than a correctness failure: after the operator left Theme Shop on its non-custom Update tab, one reopen called `buildTabs()` twice in the same 100 ms log bucket. `publishUpdateState()` and `refreshThemeShopState()` each rebuilt the current non-custom tab before `show()` reset selection. The follow-up batches those publishers and performs exactly one deferred refresh before `show()`. Unbatched update/theme changes keep their existing immediate refresh behavior. This is a reopen-cost optimization, not evidence of another fence stall; focused hardware timing remains pending.
...
The next hardware gate applied the static Zelda Minimalist package while an animated background was active. Installation and path resolution were correct: the package contained its manifest and `media/backgrounds/1.png`, the image decoded, and the runtime reported `exists=1 reloaded=1 loaded=1`. The official catalogue manifest also declares that same relative asset. The visual result was nevertheless the old animation combined with the new static theme colors.

The bottleneck was a state-lifetime defect in [WaraWaraBackground::loadImage](../projects/menu/src/widgets/WaraWaraBackground.cpp), not the theme repository. A successful still-image load replaced only `m_backgroundImage`; it did not stop the frame reader or clear `m_frames`. [currentBackground](../projects/menu/src/widgets/WaraWaraBackground.cpp) gives resident animation frames priority, so the new image was valid but unreachable. The correction decodes the replacement first, then stops and joins the reader, clears queued/resident animation frames, and publishes the still texture. Loading before clearing preserves the last working wallpaper when a replacement file is invalid. The existing GPU-idle boundary in `applyThemeResources` still protects destruction of the old textures.

The same test exposed a language-setting discoverability failure rather than a missing language implementation. `uiLanguageOverride`, locale scanning, live `I18n::setLanguage`, glyph-cache clearing, and persistence already existed. The actionable selector was the tenth System item, while a later read-only Console Language value had no explanation. `SwitchU Language` is now the first System row with an explicit `Press A` instruction; Console Language is labelled read-only. All eight shipped locale files contain the new copy. The owner completed the focused animated/static replacement, live language-change, and persisted-language checks successfully. Evidence is preserved at `.logs/gpu-upload-ring-theme-language-issues-20260828-080404`.

The normal release/sysmodule candidate compiled with every diagnostic mode disabled in 191.55 seconds (`4m40s` for the wrapper). The 42,993,206-byte pre-commit archive has SHA-256 `2D9AACB775D9E055775B1A2483514CE97854E1D7636B09E5D341EEAA26198E40`; daemon/menu/NPDM hashes are `8A6730950FDED0F887B25A2AD33814B37A633E8679DD6883B1AA727642C9F8B4`, `0D19E7FB928AFDE39D52CB65C4B241CC76871CCD9F398B1F52D889BB06717DEB`, and `5D9EFE4E476B23F963F5CDE05DEE7FC591A216BDD8653FB50CAA2F17D42FB9CF`. Binary inspection found the stale-frame replacement marker and new language copy; locale JSON parsing and `git diff --check` pass.

The implementation was committed as `f4cb0a6` and rebuilt exactly. Compilation took 93.34 seconds; the full verified deployment completed in `2m56s`. The final archive remains 42,993,206 bytes and has SHA-256 `E1ADEF5249412A2A786BE77FCC09597600C3DB80CA1728AAD3194DBE88940CFF`. Daemon/menu/NPDM sizes are 642,116/7,795,661/964 bytes with the hashes above, and all three match the connected removable FAT32 card byte-for-byte. Every diagnostic flag is false and all eight deployed locale files parse with both help keys. At the owner's request, this pass deliberately did not collect or classify new fatal/crash/ERPT records because that report pattern is specific to another user's console.
