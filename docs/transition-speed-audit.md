...
1. Complete the remaining real-state gates: optional-user modes, a second UID, first-ever save creation, physical game-card removal/update, and a controlled resume-foreground failure. Required-user launch, no-user forwarder launch, A-to-B replacement, all seven rejection fallbacks, physical sleep/wake stale fallback, and deterministic new-application failure recovery now pass on hardware.
