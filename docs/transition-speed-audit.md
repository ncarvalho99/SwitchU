...
[drainApplicationLibraryAppletForHome](../projects/daemon/src/main.cpp) now queries the running `AppletApplication`, not the unrelated flag used for applets launched by the daemon itself. The normal path pays one small `AreAnyLibraryAppletsLeft` IPC. When a child exists, libnx requests exit, force-terminates after a 100 ms grace period if needed, and SwitchU polls for at most 250 ms until AM reports no child before foreground handoff. Probe failure or a child that remains alive aborts HOME while the title still owns foreground; it never creates the memory-heavy menu in an unproved state. `[trace-home-applet]` reports probe, exit, drain, total time, results, and `left`.
...
The corrective normal release built successfully (`build ok`, 303.702 seconds; full build/deploy 6m37s) and is deployed with diagnostics off. ZIP SHA-256 is `2959045A431682A43B6D5B6FAE8BF4B3681AFBDA09BF4A9243A503FCEEDBF2DF`; daemon `exefs.nsp` is 641,574 bytes, SHA-256 `41601BCFD0C75466DB8C1DB8765F61D8309524976D0507C3D31BF4067A630949`; menu `main` is 7,794,778 bytes, SHA-256 `281F105E0BBCF10906D6F544D5081097D7FEF884A6E4743C48249FB94F0ADEE4`; NPDM remains `5D9EFE4E476B23F963F5CDE05DEE7FC591A216BDD8653FB50CAA2F17D42FB9CF`. All local/card hashes match. The linked daemon contains the HOME trace and no termination diagnostic markers. Hardware still must prove `active=1 ... left=0`, first frame, and no new crash report.
...
The implementation is deliberately fail-closed. A prepared result is reusable only when the title ID and all 16 UID bytes match, every touch/save operation completed successfully, the result was at most five seconds old when launch handling began, and a fresh control-cache read is byte-identical to the metadata used by preflight. Any missing command, IPC failure, missing cache, failed save operation, stale result, different user/title, or changed metadata invokes the former synchronous touch/save path. `[trace-preflight]` reports send/IPC/work/touch/save time, work lead or tail relative to the final command, age at action handling, completion, cache hit, and core.

The primary hardware path now passes for one required-user official title, one no-user forwarder, and A-to-B replacement. Hardware gates remain for every other NACP startup-user mode, another user, first-ever save creation, missing/failed control cache, game-card removal, same-boot title update, a failed preflight, sleep/wake, and power loss. No broad speed or integrity claim is made until those pass.
...
2. `[trace-save]`: account, device, temporary, cache, and BCAT save probes.
...
1. Complete the preflight safety matrix: optional-user modes, another UID, first-ever save creation, missing control cache, game-card removal, sleep/wake, and a failed launch. Every invalid preparation must log `cache_hit=0` and still launch through the original path where applicable.
2. Run the 30-sample matrix with `analyze-transition-traces.ps1`: 10 cold official launches, 10 installed-forwarder launches, and 10 A-to-B replacements, each with HOME return. Compare command-to-foreground and preflight lead/tail metrics.
