...
### Implemented, hardware validation pending: animated-to-static theme replacement
...
The same test exposed a language-setting discoverability failure rather than a missing language implementation. `uiLanguageOverride`, locale scanning, live `I18n::setLanguage`, glyph-cache clearing, and persistence already existed. The actionable selector was the tenth System item, while a later read-only Console Language value had no explanation. `SwitchU Language` is now the first System row with an explicit `Press A` instruction; Console Language is labelled read-only. All eight shipped locale files contain the new copy. Hardware must verify animated-to-static replacement, live language change, glyph-cache regeneration, and persistence before this gate is closed. Evidence is preserved at `.logs/gpu-upload-ring-theme-language-issues-20260828-080404`.
