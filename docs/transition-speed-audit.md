...
1. Hardware-test the corrected explicit failed-launch recovery and sleep/wake between prepare and launch; then complete the gates requiring unavailable real state: optional-user modes, a second UID, first-ever save creation, and physical game-card removal/update. The RAM-only baseline plus all seven rejection branches pass; the first edge-test harness attempt did not exercise either intended branch and is documented above.
