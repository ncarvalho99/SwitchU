...
1. Complete the remaining real-state gates: optional-user modes, a second UID, first-ever save creation, and physical game-card removal/update. Required-user launch, no-user forwarder launch, A-to-B replacement, all seven rejection fallbacks, physical sleep/wake stale fallback, and deterministic launch/resume failed-handoff recovery now pass on hardware.
