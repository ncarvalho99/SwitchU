...
The diagnostic sequence is `baseline-hit`, `missing-preparation`, `title-mismatch`, `user-mismatch`, `incomplete-preparation`, `stale-preparation`, `metadata-missing`, and `metadata-changed`. Reboot after deploying the diagnostic build, launch the same known-good installed title eight times, return with HOME after each successful foreground handoff, and require eight ordered `[diagnostic-preflight] result ... pass=1` records. Only sequence 1 may report `cache_hit=1`; sequences 2-8 must report `cache_hit=0` and still launch. The diagnostic build is forbidden for performance measurements and must be replaced immediately by a normal release after log collection.
...
1. Run the deployed RAM-only preflight matrix and require all eight cases to log `pass=1`; then complete the irreducible real-state gates: optional-user modes not installed on this console, a real second UID, first-ever save creation, game-card removal/update, sleep/wake between prepare and launch, and a failed launch. Every invalid preparation must log `cache_hit=0` and still use the original path where applicable.
