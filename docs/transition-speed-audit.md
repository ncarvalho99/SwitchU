...
The separate resume gate now has an opt-in `-ResumeFailureTest` harness. After a real suspended title returns to SwitchU, its diagnostic modal can either run the unchanged resume path or send command 47. The daemon still performs the real `appletUnlockForeground`, then substitutes synthetic libnx result `0x1F759` immediately before `appletApplicationRequestForApplicationToGetForeground`. It deliberately retains the real `AppletApplication`, `g_running`, and suspended title ID, invokes the production failure-recovery helper once, and never creates, starts, terminates, or replaces a title. The next selection can therefore prove preservation by resuming the same live session. Normal builds do not contain the diagnostic command, modal, or fault branch. Production now also refuses to close the menu if resume-command enqueue fails, captures the suspended title ID with the queued action, and records `appletUnlockForeground` result/time separately from the foreground-request IPC. Hardware validation remains pending.
...
1. Complete the remaining real-state gates: validate the controlled resume-foreground failure harness, then test optional-user modes, a second UID, first-ever save creation, and physical game-card removal/update. Required-user launch, no-user forwarder launch, A-to-B replacement, all seven rejection fallbacks, physical sleep/wake stale fallback, and deterministic new-application failure recovery now pass on hardware.
