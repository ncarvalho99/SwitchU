...
Reviewing the remaining failed-launch gate found that recovery depended on the next generic idle loop. Worse, `g_menuFastExitCount` counted every intentional title handoff forever, so after three ordinary launches the unrelated crash-loop guard could leave a failed handoff black for five seconds. A healthy `MenuReady` now resets that guard. Failed application launch or resume explicitly cold-launches the menu with transition reason `LaunchFailure`, and a failed resume no longer marks the application as foreground. This recovery is compiled in production; controlled invalid-title and sleep/wake-preflight hardware tests remain required before closing the gate.

The opt-in `-PreflightEdgeTest` sysmodule build exposes those two tests only after the normal account decision and real preflight. For sleep/wake, it leaves a modal and the prepared daemon state alive while the physical Power button sleeps the console; after wake/unlock, `Launch selected` runs the normal recency/animation/final-command path. The result must reject the older-than-five-second preparation as `stale`, run the synchronous fallback, and launch the selected title. `Invalid recovery` instead sends reserved non-installed ID `01FFFFFFFFFFF000` as the authoritative command without writing catalog or recency state. Application creation must fail, and the new production recovery path must return a reason-7 menu to its first frame exactly once. This diagnostic is compiled out of normal builds and must be replaced immediately after log collection.
...
1. Hardware-test the new explicit failed-launch recovery and sleep/wake between prepare and launch; then complete the gates requiring unavailable real state: optional-user modes, a second UID, first-ever save creation, and physical game-card removal/update. The RAM-only baseline plus all seven rejection branches pass.
