...
// SwitchU modification: see the call site in loadNro().
//
// The request file holds one line, an absolute sdmc: path to the NRO. It is
// removed as soon as it is read, whether or not the path proves usable, so a
...
static bool readSwitchURequest(void)
{
    ...
    char buf[sizeof(g_nextNroPath)];
    ssize_t got = read(fd, buf, sizeof(buf) - 1);
    close(fd);
    unlink(SWITCHU_REQUEST_PATH);

    if (got <= 0)
        return false;
    buf[got] = '\0';

    // One line, whatever line ending it was written with.
    ...
    // Anything that is not an sdmc: path is not something this should open.
    if (strncmp(buf, "sdmc:/", 6) != 0)
        return false;

    size_t len = strlen(buf);
    if (len == 0 || len >= sizeof(g_nextNroPath))
        return false;
...
void loadNro(void)
{
    ...
    if (g_nextNroPath[0] == '\0')
    {
        ...
        // here, and nothing can tell it otherwise: g_nextNroPath is only
        // ever written by homebrew already running, through the
        // envSetNextLoad ABI, so a menu starting this as an applet has no
        // way to name a target.
        //
        // The menu writes the path it wants just before starting the
        // applet, and it is consumed here: read once, then deleted, so a
        // launch that names nothing falls back to hbmenu the way it always
        // did rather than silently repeating the last choice.
        if (!readSwitchURequest())
        {
