...
void WiiUMenuApp::onUpdate(float dt) {
    ...
    if (m_perfAccumDt >= 1.0f) {
        ...
        DebugLog::log("[perf] %.1f fps  avg=%.1fms worst=%.1fms  vsync=%.1fms gpu=%.1fms  draws=%u binds=%u verts=%u dropped=%u blur=%u caps=%u uploads=%u  settings=%d(1=fullScene,2=sceneHidden) themeshop=%d",
